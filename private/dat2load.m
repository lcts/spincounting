...
while ~feof(fid)       % while not at end of file
    ...
    if length(line) >= 2
        if strcmp(line(1:2),'%?')
            if ~isempty(regexp(line, '\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}', 'once'))
                % try to read date/time in format
                % '%? <YYYY-MM-DD hh:mm:ss>'
                date = sscanf(line, '%%? %4d-%2d-%2d %2d:%2d:%2d');
            elseif ~isempty(regexp(line,'addcols','once'))
                % check if a line defines additional columns besides data
                % delete the '%?' and 'addcols' from the cell array
                addcols = textscan(line, '%s'); addcols = addcols{1}; addcols(1:2) = [];
            else
                % check for known file id strings
                % do this last, so you don't match these strings in time/date or
                % addcols lines
                if ~isempty(regexp(line,'1d',  'once')); is1d   = true; if strcmp(ftype, ''); ftype = {'1d'}; else ftype = strcat(ftype, {' '}, '1d'); end; end
                if ~isempty(regexp(line,'2d',  'once')); is2d   = true; if strcmp(ftype, ''); ftype = {'2d'}; else ftype = strcat(ftype, {' '}, '2d'); end; end
                if ~isempty(regexp(line,'2ch', 'once')); is2ch  = true; if strcmp(ftype, ''); ftype = {'2ch'}; else ftype = strcat(ftype, {' '}, '2ch'); end; end
                if ~isempty(regexp(line,'ch1', 'once')); isch1  = true; if strcmp(ftype, ''); ftype = {'ch1'}; else ftype = strcat(ftype, {' '}, 'ch1'); end; end
                if ~isempty(regexp(line,'ch2', 'once')); if strcmp(ftype, ''); ftype = {'ch2'}; else ftype = strcat(ftype, {' '}, 'ch2'); end; end
                if ~isempty(regexp(line,'akku','once')); isakku = true; if strcmp(ftype, ''); ftype = {'akku'}; else ftype = strcat(ftype, {' '}, 'akku'); end; end
                if ~isempty(regexp(line,'meta','once')); ismeta = true; if strcmp(ftype, ''); ftype = {'meta'}; else ftype = strcat(ftype, {' '}, 'meta'); end; end
            end
        %
        % GET PARAMETERS
        %
